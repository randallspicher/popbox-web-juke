...
Logger log= Logger.getLogger("browse-list.jsp"); 
...
<td class="icon">
				...
				if (image!=null){	
					...
					%>
					<img class="icon" src='icon/folder.gif'/>
					<%
				}
				%>
</td>
...
<%		out.print("<a href=\"javascript:browse(&quot;"+Utility.htmlEncode(mount+"/"+filename)+"&quot;);\">");
%>
				<%=Utility.htmlEncode(filename).replaceAll("_"," ")%> 
<%      out.print("</a>");
%>
	...
	<%	
		...
		out.print("<td class='action'><a href=\"javascript:queue(&quot;"+Utility.htmlEncode(mount+"/"+filename)+"&quot;);\" title='Add this to the Queue'><img alt='ADD' src='icon/plus.gif'></a></td>");
		...
		if (("music".equalsIgnoreCase(type)||"video".equalsIgnoreCase(type))
			...
			%>
			<td><input type="checkbox" id="itemselect" name="itemselect" value="<%=Utility.htmlEncode(mount+"/"+filename)%>"/></td> 
			<%
		}
		else {
			out.print("<td>&nbsp;</td>");
		...
		if ("photo".equalsIgnoreCase(type)){
			...
			out.print("<td class='title'>"+Utility.htmlEncode(filename)+" </td>");
